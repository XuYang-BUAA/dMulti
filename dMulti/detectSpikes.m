...
%             i
        ...
        dr_sort = sort(dr);
        dr_threshold = mean(dr_sort(1:floor(length(dr)/2)))*dr_coe;
        ...
        for i = 1:length(cluster_ind)
            templates(:,:,:,i) = mean(spikes_temp_all_test(:,:,:,locs_ch(nearest(1:S,i))),4);
            %templates(:,i) = mean(spikes_con_temp_hanning(:,nearest(1:10,i)),2);
        end
        for i=1:spike_num
